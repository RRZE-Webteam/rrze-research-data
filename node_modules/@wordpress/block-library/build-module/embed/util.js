...
const getEmbedInfoByProvider = (provider) => getBlockVariations(DEFAULT_EMBED_BLOCK)?.find(
  ({ name }) => name === provider
);
const matchesPatterns = (url, patterns = []) => patterns.some((pattern) => url.match(pattern));
const findMoreSuitableBlock = (url) => getBlockVariations(DEFAULT_EMBED_BLOCK)?.find(
...
const getAttributesFromPreview = memoize(
