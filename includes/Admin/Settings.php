...
class Settings
{
    ...
    {
        if (
                isset($_POST['rrze_clear_cache']) &&
                check_admin_referer('rrze_research_data_clear_cache')
        ) {
            $cache = new
            \RRZE\ResearchData\Services\CacheService();
            $cache->deleteAll();
            set_transient('rrze_research_cache_cleared', true,
                    30);
     ...
     *
     * Outputs a standard WordPress settings form using the Settings API.
     *
     * @return void
     */
    ...
    {
        $cleared = (bool)
        get_transient('rrze_research_cache_cleared');
        if ($cleared) {
        ...
        <div class="wrap">
            ...
            <p><?php echo esc_html__('Publication data from external sources (ORCID, PubMed, OpenAlexetc.) is cached for 12 hours to reduce API requests and improve page load times.',
                        'rrze-research-data'); ?></p>
            ...
            <?php endif; ?>
            <h2><?php echo esc_html__('Cache',
                        'rrze-research-data'); ?></h2>
            <p><?php echo esc_html__('If publications are not up to date, you can clear the cache manually. New data will be fetched from the
  source on the next page load.', 'rrze-research-data'); ?></p>
